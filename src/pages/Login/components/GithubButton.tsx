const GithubButton = ({disabled=false, complete}: IAuthButton) => {
    ...
    const receiveMessage = async (event: any) => {
        ...
        if (data?.includes("code")) {
            ...
            if(location.pathname === "/login" || complete){
                //log user in
                const res = await loginSocialCallback.mutateAsync({provider: 'github', code: code!})
                let d = res.data;
                ...
                const res = await registerSocialCallback.mutateAsync({provider: 'github', code: code!, fcmToken});
